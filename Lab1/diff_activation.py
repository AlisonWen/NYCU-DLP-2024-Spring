...
class FullyConnectedLayer(Layer):
    def __init__(self, input_shape, output_shape):
        self.W = np.random.randn(input_shape, output_shape) * 0.1
...
class Network:
    ...
    def train(self, X, Y, learning_rate, max_iter):
        for i in range(max_iter):
            ...
            if i % 100 == 0:
                loss = np.mean(np.square(Y - self.act3.output))
                ...
                # writer.add_scalar('relu-XOR', accuracy, i)
    ...
    def evaluate(self, X, Y, fig_name=None):
        ...
        print(self.act3.output.flatten())
...
if __name__ == "__main__":
    ...
    model.train(X, Y, learning_rate=0.001, max_iter=5000)
