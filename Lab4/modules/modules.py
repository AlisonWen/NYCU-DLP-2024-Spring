...
class FeatureCombiner(nn.Module):
    def __init__(self, img_chans, label_chans, out_chans):
        super(FeatureCombiner, self).__init__()
        # 1x1 convolutions to unify channel dimensions
        self.img_conv = nn.Conv2d(img_chans, out_chans, kernel_size=1)
        self.label_conv = nn.Conv2d(label_chans, out_chans, kernel_size=1)
        
    def forward(self, img_features, label_features, tfr):
        # Adjust channel dimensions
        img_features = self.img_conv(img_features)  # [batch, out_chans, H, W]
        label_features = self.label_conv(label_features)  # [batch, out_chans, H, W]
        
        # Combine features using teacher forcing ratio
        combined_features = tfr * img_features + (1 - tfr) * label_features
