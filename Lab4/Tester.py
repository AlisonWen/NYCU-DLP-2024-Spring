...
class Test_model(VAE_Model):
    ...
    def eval(self):
        ...
        for idx, (img, label) in enumerate(tqdm(val_loader, ncols=150)):
            img = img.to(self.args.device)
            label = label.to(self.args.device)
            pred_seq = self.val_one_step(img, label, idx)
            pred_seq_list.append(pred_seq)
            print(f"idx = {idx}, {len(pred_seq_list)}")
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        for i in range(1, label.shape[0]):
            # print(f"[{i}] = decoded_frame_list[-1] shape", decoded_frame_list[-1].shape)
            encoded_img = self.frame_transformation(decoded_frame_list[-1].to(self.args.device))
            encoded_label = self.label_transformation(label[i].to(self.args.device))
            z, _, _ = self.Gaussian_Predictor(encoded_img, encoded_label)
    ...
    def val_dataloader(self):
        ...
        # dataset = Dataset_Dance(root=self.args.DR, transform=transform, video_len=self.val_vi_len, mode='val')  
        # val_loader = DataLoader(dataset,
        #                           batch_size=1,
        #                           num_workers=self.args.num_workers,
        #                           drop_last=True,
        #                           shuffle=False)  
        dataset = Dataset_Dance(root=self.args.DR, transform=transform, mode='test', video_len=self.val_vi_len, partial=1.0)  
        val_loader = DataLoader(dataset,
                                  batch_size=1,
                                  num_workers=self.args.num_workers,
                                  drop_last=True,
