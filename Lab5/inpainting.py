...
class MaskGIT:
    def __init__(self, args, MaskGit_CONFIGS):
        ...
        self.model.gamma = self.model.gamma_func(args.mask_func)
    ...
    def prepare(self):
        if self.save_root is not None:
            os.makedirs("test_results/" + self.save_root, exist_ok=True)
            os.makedirs("mask_scheduling/" + self.save_root, exist_ok=True)
            os.makedirs("imga/" + self.save_root, exist_ok=True)
            for i in range(self.sweet_spot):
                os.makedirs("test_results/" + self.save_root + "/" + str(i), exist_ok=True)
                os.makedirs("mask_scheduling/" + self.save_root + "/" + str(i), exist_ok=True)
                os.makedirs("imga/" + self.save_root + "/" + str(i), exist_ok=True)
        else:
...
#mask_b: iteration decoding initial mask, where mask_b is true means mask
    def inpainting(self,image,mask_b,i, true_scheduling=False): #MakGIT inference
        ...
        with torch.no_grad():
            ...
            for step in (range(self.total_iter)):
                # print(f"before update mask_bc: {mask_bc.sum()}")
                if step == self.sweet_spot:
                    break
                ratio = (step + 1) / self.total_iter #this should be updated
                # z_indices_predict, mask_bc = self.model.inpainting(image, mask_bc, ratio)
                z_indices_predict, mask_bc = self.model.inpainting(z_indices_predict, mask_bc, ratio, true_scheduling, mask_b)
                z_indices_predict = torch.where(mask_bc, z_indices_predict, z_indices)
                z_indices_predict = torch.clamp(z_indices_predict, 0, 1023)
                # print(f"step={step}, ratio={ratio}")
                ...
                # print(f"ratio {ratio}, gamma(ratio): {self.model.gamma(ratio)}, mask_len: {mask_bc.sum()}")
            # print('-' * 50)
            # a=1/0
            ##decoded image of the sweet spot only, the test_results folder path will be the --predicted-path for fid score calculation
                vutils.save_image(dec_img_ori, os.path.join("test_results/" + self.save_root + "/" + str(step), f"image_{i:03d}.png"), nrow=1) 

                #demo score 
                vutils.save_image(maska, os.path.join("mask_scheduling/" + self.save_root + "/" + str(step), f"test_{i}.png"), nrow=10) 
                vutils.save_image(imga, os.path.join("imga/" + self.save_root + "/" + str(step), f"test_{i}.png"), nrow=7)
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='./transformer_checkpoints', help='load ckpt')
    
    #dataset path
    parser.add_argument('--test-maskedimage-path', type=str, default='../dataset/lab5_dataset/cat_face/masked_image', help='Path to testing image dataset.')
    parser.add_argument('--test-mask-path', type=str, default='../dataset/lab5_dataset/mask64', help='Path to testing mask dataset.')
    #MVTM parameter
    ...
    parser.add_argument('--true-scheduling', action=argparse.BooleanOptionalAction)
    ...
    i=0
    for image, mask in tqdm(zip(t.mi_ori, t.mask_ori), ncols=200):
        # print(f"i=[{i}]")
        image = image.to(device=args.device)
        mask = mask.to(device=args.device)
        mask_b = t.get_mask_latent(mask)       
        maskgit.inpainting(image,mask_b,i, args.true_scheduling)
        i += 1
